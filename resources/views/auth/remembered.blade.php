<x-layouts.auth title="Welcome Back">
    ...
    <div class="w-11/12 flex flex-col gap-6 items-center">
        <!-- Account Info Card -->
        <div class="w-full p-5 rounded-xl border-[1.5px] border-slate-200 bg-slate-50/50 flex flex-col gap-2 text-center">
            ...
            <div class="inline-flex items-center justify-center gap-1.5 self-center mt-2 px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider text-[#4361EE] bg-[#f0f3ff] border-[1.5px] border-[#4361EE]/20">
                <span>{{ $user->role ?? 'student' }}</span>
            </div>
        </div>

        <div class="w-full flex flex-col gap-3">
            <a href="{{ route('home') }}" class="primary-btn text-center flex items-center justify-center font-bold no-underline w-full">
                Go to Dashboard
            </a>
            ...
            <form id="logoutForm" action="{{ route('logout') }}" method="POST" class="w-full m-0">
                @csrf
                <button type="submit" class="secondary-btn w-full font-bold text-[10px] uppercase tracking-wider transition-all cursor-pointer">
