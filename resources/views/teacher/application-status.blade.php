<x-layouts.auth title="Teacher account review">
    <div class="w-11/12 text-center space-y-5">
        ...
        @if($user->teacher_approval_status === 'rejected')
            ...
            @if($user->teacher_rejection_reason)<div class="p-4 border border-red-200 bg-red-50 rounded-xl text-left">{{ $user->teacher_rejection_reason }}</div>@endif
        @else
        ...
        @endif
        <form action="{{ route('logout') }}" method="POST">@csrf<button class="secondary-btn w-full">Sign out</button></form>
    </div>
</x-layouts.auth>
