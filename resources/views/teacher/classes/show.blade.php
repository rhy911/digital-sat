<x-layouts.student :user="auth()->user()" :title="$classroom->name" header-type="progress">
    ...
    <div class="page-heading"><div><span class="status-chip status-chip--{{ $classroom->status }}">{{ ucfirst($classroom->status) }}</span><h1>{{ $classroom->name }}</h1><p>{{ $classroom->description ?: 'Manage roster and assigned work.' }}</p></div>
        <div class="join-code"><span>Student join code</span><strong>{{ $classroom->join_code }}</strong>@if($classroom->status === 'active')<label>Join link<input readonly value="{{ route('student.classes.join-link', $classroom->join_code) }}" onclick="this.select()"></label><form method="POST" action="{{ route('teacher.classes.rotate-code', $classroom) }}">@csrf<button class="text-button">Rotate code</button></form>@endif</div>
    </div>
    @if($classroom->status === 'active')<details class="create-disclosure"><summary>Edit class details</summary><form method="POST" action="{{ route('teacher.classes.update', $classroom) }}" class="inline-form">@csrf @method('PUT')<label>Class name<input name="name" value="{{ $classroom->name }}" required maxlength="150"></label><label>Description<input name="description" value="{{ $classroom->description }}" maxlength="2000"></label><button class="class-button class-button--primary">Save</button></form></details>@endif
    <div class="class-tabs" role="navigation" aria-label="Class sections"><a href="#roster">Roster</a><a href="#assignments">Assignments</a></div>
    <section id="roster" class="class-panel">
        <div class="section-heading"><div><h2>Roster</h2><p>{{ $classroom->active_memberships_count }} active students</p></div></div>
    ...
    <section id="assignments" class="class-panel">
        <div class="section-heading"><div><h2>Assignments</h2><p>Publish an owned active test to everyone in this class.</p></div></div>
        ...
        <details class="create-disclosure"><summary>Create assignment</summary>
            <form method="POST" action="{{ route('teacher.assignments.store', $classroom) }}" class="form-grid">@csrf
                ...
                <div class="form-action"><button class="class-button class-button--primary">Save draft</button></div>
            </form>
        </details>@endif
