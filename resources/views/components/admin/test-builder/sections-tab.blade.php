    ...
    <div class="space-y-5">
        ...
        <div id="sectionsEmptyState"
            ...
            <div class="p-16 text-center">
                ...
                <p class="text-slate-600 mb-8 max-w-sm mx-auto text-sm leading-relaxed">Add a section to a test, then attach the modules students will take.</p>
                <button type="button"
        ...
        <div id="sectionsTableContainer"
            ...
            <div
                ...
                <div class="flex items-center gap-4 flex-wrap md:flex-nowrap">
                    <button class="inline-flex min-h-10 items-center gap-2 rounded-lg bg-indigo-600 px-3 py-2 text-xs font-bold text-white hover:bg-indigo-700" x-on:click="$dispatch('open-offcanvas', 'createSectionOffcanvas')"><i class="bi bi-plus-lg" aria-hidden="true"></i>Create section</button>
                    @if(auth()->user()->role === 'teacher')
                        <div class="flex items-center gap-2 bg-white px-3 py-1.5 rounded-lg border border-slate-200">
                            <label for="sectionsShowSharedToggle" class="text-xs font-bold text-slate-600 cursor-pointer select-none">Show shared</label>
                            <input type="checkbox" id="sectionsShowSharedToggle" class="w-4 h-4 text-indigo-600 border-slate-300 bg-white rounded cursor-pointer sections-show-shared-toggle">
                        </div>
                    @endif
                    <div class="relative">
        ...
        <form id="sectionForm" class="space-y-6">
            ...
            <div>
                <label for="sectionTest"
                    class="text-xs font-extrabold text-slate-500 tracking-wider uppercase mb-2 block">Parent test <span class="text-rose-500">*</span></label>
                <select
            ...
            <div>
                <label for="sectionType"
                    class="text-xs font-extrabold text-slate-500 tracking-wider uppercase mb-2 block">Section type <span class="text-rose-500">*</span></label>
                <select
            ...
            <div class="flex items-center ml-2">
                <label class="flex items-center group cursor-pointer">
                    <input type="checkbox" name="is_public" value="1" class="w-4 h-4 text-emerald-600 border-slate-300 bg-white rounded-md cursor-pointer">
                    <span class="ml-2.5 text-xs font-extrabold text-slate-500 group-hover:text-emerald-600 uppercase tracking-wider">Public visibility</span>
                </label>
            </div>
            ...
            <div class="bg-amber-50 border border-amber-200 rounded-xl p-5 flex gap-4 items-start shadow-xs">
                ...
                <div>
                    <h6 class="text-xs font-extrabold text-amber-800 uppercase tracking-wider mb-1">Standardized SAT Structure</h6>
                    <p class="text-xs text-amber-700 leading-relaxed mb-0 font-medium">
                        Section order is fixed for Digital SAT: <strong class="text-amber-800 font-bold">Reading & Writing</strong> (order 1), <strong class="text-amber-800 font-bold">Math</strong> (order 2). You can only add one section of each type per test.
                    </p>
                </div>
            </div>
