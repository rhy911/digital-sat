...
<div class="option flex flex-col items-start text-start {{ $compact ? '' : 'w-100 mb-3' }}"
    ...
    <div class="flex justify-between items-start w-full">
        ...
        <button type="button" x-data x-on:click.prevent="$dispatch('open-modal', '{{ $modalId }}')" class="text-gray-400 hover:text-rose-600 transition-colors mt-1 ml-2" title="Delete Attempt">
    ...
    <div class="status-badge bg-amber-100 text-amber-800 border-amber-200 mt-2">⏳ In Progress</div>
    
    @if($moduleUlid)
        <a href="{{ route('engine.session', ['ulid' => $moduleUlid]) }}?attempt={{ $userTest->ulid }}" class="text-brand hover:underline text-lg font-bold ml-auto mt-2">Resume Test</a>
    @else
        ...
        <x-slot name="footer">
            <div class="flex justify-end gap-3 w-full">
                ...
                <form action="{{ route('my-practice.destroy', $userTest->ulid) }}" method="POST" class="m-0">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="px-4 py-2 bg-rose-600 text-white rounded hover:bg-rose-700 transition-colors">Delete</button>
                </form>
            </div>
        </x-slot>
    </x-ui.modal>
