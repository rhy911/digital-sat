    ...
    <div class="student-workspace student-workspace--narrow">
        <a href="{{ route('dashboard') }}" class="back-link">
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none"
                ...
                class="inline mr-1" style="vertical-align: -2px;">
        ...
        <div class="class-panel" style="display: flex; align-items: center; gap: 24px; padding: 24px;">
            ...
            <div>
                <span class="status-chip status-chip--active"
                    style="margin-bottom: 6px;">{{ ucfirst($user->role) }}</span>
                <h1 style="margin: 0 0 4px; font-size: 1.50rem; font-weight: 850; color: var(--cw-ink-strong);">
                    {{ $user->name ?: $user->username }}</h1>
                <p style="margin: 0; color: var(--cw-muted); font-size: 0.9rem;">Member since
        ...
        <section class="class-panel" aria-labelledby="profile-info-heading">
            <h2 id="profile-info-heading"
                style="margin: 0 0 6px; color: var(--cw-ink-strong); font-size: 1.25rem; font-weight: 800;">Profile
                Information</h2>
            <p style="margin: 0 0 20px; color: var(--cw-muted); font-size: 0.9rem;">Update your account's profile
                information.</p>
                    ...
                    <label style="opacity: 0.85;">Email address (Read-only)
                        <div style="position: relative;">
                            <input type="email" name="email" value="{{ $user->email }}" readonly
                                style="background-color: var(--cw-surface-soft); cursor: not-allowed; padding-right: 36px;">
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24"
                                fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                                stroke-linejoin="round"
                                style="position: absolute; right: 12px; top: 50%; transform: translateY(-50%); color: var(--cw-muted);">
        ...
        <section class="class-panel" aria-labelledby="password-heading">
            <h2 id="password-heading"
                style="margin: 0 0 6px; color: var(--cw-ink-strong); font-size: 1.25rem; font-weight: 800;">Update
                Password</h2>
            <p style="margin: 0 0 20px; color: var(--cw-muted); font-size: 0.9rem;">Ensure your account is using a long,
